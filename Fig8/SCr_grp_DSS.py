...
for g in gender_list:
    ...
    plt.savefig(outpath + g + '_SCr_grp_renal_disease_CIC.pdf')
    ...
    plt.text(0.5, 0.1, f'p-value: {p_value:.4f}', fontsize=12, transform=plt.gca().transAxes)    
    plt.title('OS')
    plt.xlabel('Time (year)')
    plt.ylabel('Survival Probability')
    plt.savefig(outpath + g + '_SCr_grp_DSS.pdf')
