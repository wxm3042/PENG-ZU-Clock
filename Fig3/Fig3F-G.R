...
a=60
  ...
  for(tg in c('y', 'o')){
    if(tg=='y'){
      disease_pdata <- disease_pdata_raw %>% 
        filter(age<a)
    }else{
      disease_pdata <- disease_pdata_raw %>% 
        filter(age>=a)
    ...
    ggsave(
      filename = paste0(outpath, 'age', a, '_', tg,'_disease_ratio_in_agegaps_group_sig.pdf'),
      plt,
      width = 3,
      height = 3
    )
