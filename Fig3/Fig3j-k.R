...
age_gap_path <- 'output/3.clock/changsha_clock/elastic_net_scale/merge_bj_cs/age_gaps_with_disease.xls'
...
for(g in names(raw_Dat_path_list)){
  ...
  if(g == 'bj'){
    ...
    dat_list[[g]] <- read_excel(raw_Dat_path_list[[g]]) %>% 
      select(1, 36, 6) %>% 
      rename(patient_id=1, q_id=2, gender=3) %>% 
      filter(!is.na(q_id)) %>% 
...
write_xlsx(
  tmp_out_type,
  paste0(outpath, 'patient_group.xlsx')
)
