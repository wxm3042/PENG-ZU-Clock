...
age_gap_path <- 'V20250427/output/Fig5/UKB/predict_UKB/age_gaps.xlsx'
...
plt <- ggplot(dat2, aes(Age, predict_age, color=type)) +
  ...
  labs(x='Chronological age', y='Biological age', color='') +
  ylim(0,200) +
  theme_bw() +
  theme(
...
ggsave(
  filename = paste0(outpath, 'cor_plot_sigma1.tiff'),
  plt,
  device = "tiff",
  compression = "lzw",
  dpi = 300,
...
grp_dath <- merge(dat2, death_dat, by.x='pt_id', by.y='eid')
grp_dath <- grp_dath %>% 
  mutate(type=ifelse(type=='Decelerated', 'Decelerated', type)) %>% 
  mutate(time=time/31)
...
ggsave(
  filename = paste0(outpath, 'total_surv.pdf'),
  p_surv,
  width = 4,
  height = 3.5
)
