...
expdat_cs <- read.xlsx(exp_path_list[['cs']]) %>% 
  rename(pt_id=1, gender=6) %>% 
  mutate(gender=ifelse(gender=='男', 1, 2))


delta_age_raw <- read.xlsx(age_path)
# annot_df <- read_excel(annot_path)
annot_df <- read.xlsx(dat_path_list[['bj']], sheet='q_fea') %>% 
  fill(name, group)
# process -----------------------------------------------------------------

for(g in young_old_list) {
  if (g == 'Y') {
    delta_age <- delta_age_raw  %>%
      filter(Age < 60) %>%
      rename(samp_id = 1) %>%
  ...
  for (i in group_annot$group) {
    ...
    pdata_cs <- expdat_cs %>%
      select(pt_id, all_of(cs_id_use), `s3_A4.年龄(岁)`, gender) %>%
    ...
    if (type == 'categorical_variable') {
      fea_list_tmp <-
        filter(annot_df, group == gsub(' $', '', i)) %>% pull(choose)
      ...
      if (length(unique(pdata$feature)) != 1) {
        fit_res <-
          lm(age_gaps ~ feature + age + gender + sour, data = pdata)
  ...
  write.xlsx(res_df,
             paste0(
               outpath,
               g,
               '_questionary_features_cor_with_delta_age_all.xlsx'
             ))
  ...
  write.xlsx(sig_res,
             paste0(
               outpath,
               g,
               '_questionary_features_cor_with_delta_age.xlsx'
             ))
}
