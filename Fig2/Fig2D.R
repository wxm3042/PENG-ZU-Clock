...
outpath <- 'output/0.figure/figure2/all_line_gender/'

type_df <- read_excel(type_path) %>% 
  mutate(id=ifelse(id=='Time_6_meters_FS','gait_speed_FS', id)) %>% 
  mutate(id=ifelse(id=='Time_6_meters','gait_speed', id))
...
for(g in names(exp_dat_path_list)){
  tdata_raw <- read_excel(exp_dat_path_list[[g]])
  colnames(tdata_raw) <- gsub('Time_6_meters_FS','gait_speed_FS',colnames(tdata_raw))
  colnames(tdata_raw) <- gsub('Time_6_meters','gait_speed',colnames(tdata_raw))
  tdata <- tdata_raw %>% 
...
ggsave(
  filename = paste0(outpath,  'all_fea_dotplot_gender.pdf'),
  out_plt,
  width = 40,
  height = 15
)
